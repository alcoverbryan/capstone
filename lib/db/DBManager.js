...
export default class DBManager {
    constructor(path) {
        this.sequelize = new Sequelize({ dialect: "sqlite", storage: path, logging: false });
    }
    ...
    defineModels() {
        ...
        this.branches = this.sequelize.define("branches", {
            user_id: {
        ...
        this.dailyDeposits = this.sequelize.define("dailyDeposits", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.pendingPasswords = this.sequelize.define("pendingPasswords", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.pendingUsers = this.sequelize.define("pendingUsers", {
            first_name: {
                type: DataTypes.STRING,
                allowNull: false,
            },
        ...
        this.dailyDips = this.sequelize.define("dailyDips", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.fuelDeliveries = this.sequelize.define("fuelDeliveries", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.dailySalesVolumes = this.sequelize.define("dailySalesVolumes", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.dailySalesPos = this.sequelize.define("dailySalesPos", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.pumpPrices = this.sequelize.define("pumpPrices", {
            user_id: {
                type: DataTypes.INTEGER,
                allowNull: false,
                references: {
        ...
        this.users.hasMany(this.branches, {
            foreignKey: "user_id",
        });
        ...
        this.users.hasMany(this.pendingPasswords, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.dailyDeposits, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.pendingUsers, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.dailyDips, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.fuelDeliveries, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.dailySalesVolumes, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.dailySalesPos, {
            foreignKey: "user_id",
        });
        this.users.hasMany(this.pumpPrices, {
            foreignKey: "user_id",
        });


        this.branches.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        ...
        this.pendingPasswords.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.dailyDeposits.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.pendingUsers.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.dailyDips.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.fuelDeliveries.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.dailySalesVolumes.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.dailySalesPos.belongsTo(this.users, {
            foreignKey: "user_id",
        });
        this.pumpPrices.belongsTo(this.users, {
            foreignKey: "user_id",
        });
    }

    register(first_name, last_name, gender, permanent_address, email, password, username, phone_num, position, branch_id, otp) {
        return new Promise(async (resolve) => {
            try {
            
                if (!first_name || !last_name ||!gender || !permanent_address || !password) {
    ...
    createPendingUser( first_name, last_name, gender, permanent_address, email, password, username, phone_num, position, branch_id, otp) {
        return new Promise(async (resolve) => {
            try {
                ...
                if (!first_name || !last_name ||!gender || !permanent_address || !password) {
                    ...
                    const newRegister = await this.pendingUsers.create({
                        first_name: first_name,
                        last_name: last_name,
                        gender: gender,
                        permanent_address: permanent_address,
    ...
    getPendingUser() {
        return new Promise(async (resolve) => {
            let rs_register = await this.pendingUsers.findAll();
            let register = [];
        ...
        return new Promise(async (resolve) => {
            try {
                if (!user_id || !branch_name || !location) {
                    ...
                    const newRegister = await this.branches.create({
                        user_id: user_id,
    ...
    getBranch() {
        return new Promise(async (resolve) => {
            let rs_branch = await this.branches.findAll();
            let branch = [];
    ...
    addChargeAccounts(user_id, date, soa, cust_name, volume,  ammount, terms, bank, check_no, ammount_paid, ewt, cr_num, date_collected) {
        console.log(`hello ${user_id, date, soa, cust_name, volume,  ammount, terms}`)
        return new Promise(async (resolve) => {
            try {
                if (!user_id || !soa) {
                    console.log("Please provide all required fields.");
    ...
    addActualPOS(user_id, date, time, reference_id, actualPOS, overShortage) {
        return new Promise(async (resolve) => {
            try {
                if (!user_id || !date) {
                    console.log("Please provide all required fields.");
    ...
    addDailyDeposit(user_id, dateDeposited, shiftDate, source, cashier, type, bankDepositedTo, subtotal) {
        return new Promise(async (resolve) => {
            try {
               
                    const newChargeAccount = await this.dailyDeposits.create({
                        user_id: user_id,
                        ...
                        cashier: cashier,
                        type: type,
                        bankDepositedTo: bankDepositedTo,
                        subtotal: subtotal,
                    });
                    resolve(newChargeAccount.id);
            } catch (error) {
    ...
    async getDailyDeposit() {
        try {
            let rs_actual_pos = await this.dailyDeposits.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
    ...
    async updateRegisterDetails(id, register_data) {
        try {
            ...
            if (register_to_Update) {
                register_to_Update.first_name = register_data.first_name;
                register_to_Update.last_name = register_data.last_name;
                register_to_Update.gender = register_data.gender;
    ...
    pendingPasswordChanges(register_id, new_password) {
        return new Promise(async (resolve) => {
            try {
                const newPassword = await this.pendingPasswords.create({
                    user_id: register_id,
                    new_password: new_password,
                });
                resolve(newPassword.id);
            } catch (error) {
                resolve(0);
            }
        });
    ...
    getPendingPasswords() {
        return new Promise(async (resolve) => {
            try {
                const rs_pending = await this.pendingPasswords.findAll({
                    include: [
                        {
                            model: this.users,
                ...
                resolve(this.rsToArray(rs_pending));
            } catch (error) {
    ...
    async deletePendingPasswords(id) {
        try {
            await this.pendingPasswords.destroy({
                where: {
                    user_id: id,
                },
            });
    ...
    async deletePendingUser(id) {
        try {
            await this.pendingUsers.destroy({
                where: {
                    id: id,
                },
            });
    ...
    addDailyDip(user_id, day, date, vpn, fsg, vpnR, fsd, vpnD) {
        return new Promise(async (resolve) => {
            try {
                    const newChargeAccount = await this.dailyDips.create({
                        user_id: user_id,
                        day: day,
                        date: date,
                        vpn: vpn,
    ...
    async getDailyDip() {
        try {
            let rs_actual_pos = await this.dailyDips.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
            ...
            console.error("Error fetching daily dip:", error);
            throw error;
        }
    } 
    addFuelDeliveries(user_id, fuelDate, fuelVpnPlus, fuelFsg, fuelVpnR, fuelFsd, fuelVpnD) {
        return new Promise(async (resolve) => {
            try {
                    const newChargeAccount = await this.fuelDeliveries.create({
                        user_id: user_id,
                        fuelDate: fuelDate,
                        fuelVpnPlus: fuelVpnPlus,
                        fuelFsg: fuelFsg,
                        fuelVpnR: fuelVpnR,
                        fuelFsd: fuelFsd,
                        fuelVpnD: fuelVpnD,
    ...
    async getFuelDeliveries() {
        try {
            let rs_actual_pos = await this.fuelDeliveries.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
    ...
    addDailySalesVolume(user_id, day, date, vpnPlus, fsg, vpnR, fsd, vpnD) {
        return new Promise(async (resolve) => {
            try {
                    const newChargeAccount = await this.dailySalesVolumes.create({
                        user_id: user_id,
                        day: day,
                        date: date,
                        vpnPlus: vpnPlus,
    ...
    async getDailySalesVolumes() {
        try {
            let rs_actual_pos = await this.dailySalesVolumes.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
    ...
    addDailySalesPos(user_id, posDate, posVpnPlus, posFsg, posVpnR, posFsd, posVpnD) {
        return new Promise(async (resolve) => {
            try {
                    const newChargeAccount = await this.dailySalesPos.create({
                        user_id: user_id,
                        posDate: posDate,
                        posVpnPlus: posVpnPlus,
                        posFsg: posFsg,
                        posVpnR: posVpnR,
                        posFsd: posFsd,
                        posVpnD: posVpnD,
    ...
    async getDailySalesPos() {
        try {
            let rs_actual_pos = await this.dailySalesPos.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
    ...
    addPumpPrices(user_id, pumpDay, pumpDate, pumpVpnPlus, pumpFsg, pumpVpnR, pumpFsd, pumpVpnD) {
        return new Promise(async (resolve) => {
            try {
                    const newChargeAccount = await this.pumpPrices.create({
                        user_id: user_id,
                        pumpDay: pumpDay,
                        pumpDate: pumpDate,
                        pumpVpnPlus: pumpVpnPlus,
                        pumpFsg: pumpFsg,
                        pumpVpnR: pumpVpnR,
                        pumpFsd: pumpFsd,
                        pumpVpnD: pumpVpnD,
    ...
    async getPumpPrices() {
        try {
            let rs_actual_pos = await this.pumpPrices.findAll({
                include: [
                    {
                        model: this.users,
                        attributes: [
    ...
    async updateCreateDeviceDetails(id, branchData) {
        try {
            ...
            if (existingBranch) {
                existingBranch.branch_name = branchData.branch_name;
            ...
            console.error(`Error updating branch details with ID ${id}: ${error.message}`);
        }
    }

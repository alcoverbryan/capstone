...
export default function NavbarMain({
    ...
    const [previousPath, setPreviousPath] = useState(null); // State to store the previous path
    const router = useRouter();

    // Update the previous path when the component mounts or when the route changes
    useEffect(() => {
    ...
    const navigateTo = (path) => {
        if (path === "UserProfile") {
            if (userLogIn) {
                router.push({
                    ...
                    query: {
                        ...
                        back: previousPath, // Include the previous path
                    },
                });
            } else {
            ...
            router.push({
                pathname: "/Settings",
                query: { 
                    pending: JSON.stringify(displayAllPending), 
                    back: previousPath,
                    userLogIn: JSON.stringify(userLogIn),
                    pendingUsers: JSON.stringify(getPendingUsers),
                }, 
            });            
        }
    };
            ...
            <div className="flex flex-row w-full justify-end">
                                        ...
                                        <ul>
                                            {allBranch.map((branch) => (
                                                <li key={branch.id} className="mb-2">
                                                    <p className="font-semibold text-nowrap border-0">
                                                        {branch.branch_name}
                                                    </p>
                                                </li>
                                            ))}
                ...
                <div className="relative">
                    <div className="flex items-center gap-1">
                        <button className="text-md text-black font-semibold" onClick={toggleUserDropdown}>{userLogIn.first_name}</button>
                        <button className="text-md text-black font-semibold" onClick={toggleUserDropdown}>{userLogIn.last_name}</button>
                    </div>
                    {isUserDropdownOpen && (
                        <div className="absolute right-0 mt-3 text-black border bg-white shadow-lg rounded-lg w-48">
                            <ul>
                                <li
                                    className="flex items-center px-4 justify-start hover:bg-gray-100 cursor-pointer"
                                    onClick={() => navigateTo("UserProfile")}
                                >
                                    <ProfileIcon className="w-5 h-5" />
                                    <h1 className="block px-4 py-2 text-gray-700">Profile</h1>
                                </li>
                                {!hideForUser && (
                                    <li
                                    className="flex items-center px-4 justify-start hover:bg-gray-100 cursor-pointer"
                                    ...
                                    >
                                        ...
                                        <h1 className="block px-4 py-2 text-gray-700">Settings</h1>
                                    </li>
                                )}
                                <li className="flex items-center px-4 justify-start hover:bg-gray-100">
                                    <PowerIcon className="w-4 h-4" />
                                    <a href="/Account/Login" className="block px-4 py-2 text-gray-700">
                                        Logout
                                    </a>
                                </li>
                            </ul>

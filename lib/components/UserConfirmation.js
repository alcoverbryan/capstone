...
import UserConfirmationModal from "./SettingsModal/UserConfirmationModal";

export default function UserConfirmation({ pendingUserData, searchInput, userLogIn, handleClose }) {
    ...
    const closeDetails = () => {
        setSelectedUser(null); // Close the modal by setting selectedUser to null
    };
    ...
    return (
        <div className="mt-10 p-6 flex flex-col items-center justify-center">
                        ...
                        {pendingUserData
                            ...
                            .map((user) => (
                                <tr key={user.id} className="border-b">
                                    ...
                                    <td
                                        className="px-4 py-2 text-[14px] text-dark"
                                        onClick={() => handleUserClick(user)} // Set the selected user when clicked
                                    >
            ...
            {/* Pass selectedUser as a prop to UserConfirmationModal */}

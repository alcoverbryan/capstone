...
export default function AdminDashboard({ allBranch, allRegister, userLogIn, hideForUser }) {
    ...
    const renderContent = () => {
        if (showAllMembers) {
            ...
            return <AllMembersComponent handleBack={handleBack} allRegister={filteredEmployees} allBranch={allBranch} userLogIn={userLogIn} searchInput={searchInput} hideForUser={hideForUser} />;
        }
            ...
            <div className="flex items-center">
                {!showAllMembers && (
                    <div className="flex flex-col space-y-10 ">
                        <div
                            className="bg-gray-200 p-7 max-w-60 h-20 rounded-lg shadow-md border cursor-pointer hover:bg-red-500 hover:text-white"
                            onClick={() => {
                                setSelectedSection('Fuel Sales Reports');
                                setShowAllMembers(false); 
                            }}
                        >
                            <p className='text-lg'>Fuel Sales Reports</p>
                        </div>
                        <div
                            className="bg-gray-200 p-4 max-w-60 h-20 rounded-lg shadow-md border cursor-pointer hover:bg-red-500 hover:text-white"
                            onClick={() => {
                                setShowAllMembers(true); 
                            }}
                        >
                            <p className='text-lg'>Employees<br/>
                        ...
                        >
                            <p className='text-lg'>Fuel Trend Graph</p>
                        </div>
                    </div>
                )}

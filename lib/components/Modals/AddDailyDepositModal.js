...
export default function AddDailyDepositModal({ userLogIn, handleDailyDepositSave }) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (response.ok) {
                console.log("Daily deposits added successfully");
                handleDailyDepositSave();
                router.push(`/Users/${userLogIn.id}?view=daily_deposit`);
            } else {
                const errorData = await response.json();
                console.error("Failed to save daily deposits", errorData);
            }
        } catch (error) {
            ...
            <div className="flex flex-col items-center">
                <form className="w-full" onSubmit={handleSubmit}>
                                ...
                                <tr key={index}>
                                    <td className="border border-black">
                                        <input
                                            ...
                                            onChange={(e) =>
                                                handleRowChange(index, "dateDeposited", e.target.value)
                                            }
                                        />
                                    </td>
                                    <td className="border border-black">
                                        <input
                                            ...
                                            className="w-full border-none focus:ring-0 focus: outline-none px-1 py-0"
                                            onChange={(e) =>
                                                handleRowChange(index, "shiftDate", e.target.value)
                                            }
                                        />
                                    </td>
                                    <td className="border border-black">
                                        <select
                                            ...
                                            className={`w-auto border-none focus:ring-0 focus:outline-none px-1 py-0 ${
                                                row.source === "" ? "text-gray-400" : "text-black"
                                            }`} 
                                        >
                                            <option disabled value="" className="text-gray-400">Select Source</option>
                                    ...
                                    <td className="border border-black">
                                        <input
                                            ...
                                            className="w-auto border-none focus:ring-0 focus: outline-none px-1 py-0"
                                            onChange={(e) =>
                                                handleRowChange(index, "bankDepositedTo", e.target.value)
                                            }
                                        />
                                    </td>
                                    <td className="border border-black">
                    ...
                    <div className="flex items-center justify-center mt-4">
                        <button className="bg-red-500 hover:bg-red-600 text-white px-20 py-1 rounded-md" type="button" onClick={addRow}>
                        Add Row
                        </button>
